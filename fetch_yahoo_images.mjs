...
(async () => {
  ...
  console.log(`Starting Yahoo scrape for ${items.length} items...`);
  ...
  async function scrapeItem(title, i) {
    ...
    try {
      ...
      if (title.toLowerCase().includes("seed")) query = title + " agriculture farm field crop";
        ...
        title.toLowerCase().includes("urea") ||
        title.toLowerCase().includes("dap") ||
        title.toLowerCase().includes("npk")
      )
        query = title + " fertilizer bag agriculture";
      else if (title.toLowerCase().includes("plant")) query = title + " nursery agriculture";
      else query = title + " agriculture real product";

      await page.goto(
        "https://images.search.yahoo.com/search/images?p=" + encodeURIComponent(query),
        { waitUntil: "domcontentloaded", timeout: 15000 },
      );

      const imgUrl = await page.evaluate(() => {
        const imgs = document.querySelectorAll("#sres img");
        for (let img of imgs) {
      ...
      if (imgUrl) {
        ...
        mappings[title] =
          "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c7/Groundnuts_yield%2C_World%2C_2024_%28cropped%29.svg/960px-Groundnuts_yield%2C_World%2C_2024_%28cropped%29.svg.png";
  ...
  console.log("Done mapping Yahoo images via Puppeteer!");
})();
