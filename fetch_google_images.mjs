...
(async () => {
  ...
  for (const row of rowsRaw) {
    ...
    if (titleMatch) {
      ...
      if (query.toLowerCase().includes("seed") || query.toLowerCase().includes("rhizome"))
        query += " agriculture field crop";
      else if (
        query.toLowerCase().includes("urea") ||
        query.toLowerCase().includes("dap") ||
        query.toLowerCase().includes("npk") ||
        query.toLowerCase().includes("compost")
      )
        query += " fertilizer bag";
      else if (
        query.toLowerCase().includes("pump") ||
        query.toLowerCase().includes("sprayer") ||
        query.toLowerCase().includes("cutter") ||
        query.toLowerCase().includes("tractor")
      )
        ...
        if (images && images.length > 0) {
          ...
          let selected = images.find(
            (img) =>
              ...
              !img.url.includes("shutterstock") &&
              !img.url.includes("istock") &&
              !img.url.includes("x.com"),
          );
          ...
          mappings[title] =
            "https://upload.wikimedia.org/wikipedia/commons/thumb/a/a3/Eq_it-na_pizza-margherita_sep2005_sml.jpg/600px-Eq_it-na_pizza-margherita_sep2005_sml.jpg"; // obvious error
        }
      } catch (err) {
