...
content = content.replace(/export const IMAGE_MAPPINGS: Record<string, string> = \{[\s\S]*?\};\n?/, '');

// Replace imageUrl logic
// From: let imageUrl = IMAGE_MAPPINGS[name] || "";
// To: let imageUrl = `/images/products/${id}.jpg`;
content = content.replace(/let imageUrl = IMAGE_MAPPINGS\[name\] \|\| .*?;/g, 'let imageUrl = `/images/products/${id}.jpg`;');
content = content.replace(/let imageUrl = IMAGE_MAPPINGS\[name\];/g, 'let imageUrl = `/images/products/${id}.jpg`;');
