  ...
  for (const row of rowsRaw) {
    ...
    if (titleMatch) {
      ...
      if (!url) {
        ...
        else if (title.toLowerCase().includes("fertilizer") || title.toLowerCase().includes("urea"))
          url = await getWikiImage("fertilizer bag agriculture");
        else if (title.toLowerCase().includes("tractor") || title.toLowerCase().includes("sprayer"))
          url = await getWikiImage("agricultural machinery");
      ...
      if (url) {
        ...
        mappings[title] =
          "https://upload.wikimedia.org/wikipedia/commons/thumb/1/13/Tractor_in_a_field.jpg/600px-Tractor_in_a_field.jpg";
      }
    }
  }
