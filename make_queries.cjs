...
const searchQueries = {};
const negativeKeywords = ['molecule', 'chemical', 'structure', 'formula', 'vector', 'illustration', 'drawing', 'cartoon', 'clipart', 'youtube', 'ytimg', 'logo', 'poster', 'academy', 'step-by-step', 'tutorial', 'dreamstime', 'shutterstock', 'alamy', 'png', 'svg'];
...
for (const p of products) {
  ...
  if (p.category === 'seeds') {
    ...
    else if (query.includes('Grafted Plant')) query = `${p.name.split('Grafted Plant')[0]} sapling agriculture photography`;
    else if (query.includes('Sapling')) query = `${p.name.split('Sapling')[0]} plant sapling agriculture photography`;
    else query = `${p.name.split('(')[0]} macro photography`;
  } else if (p.category === 'fertilizers') {
    if (query.includes('Urea Liquid') || query.includes('Nano')) query = "IFFCO Nano liquid fertilizer bottle";
    ...
    else if (query.includes('MOP') || query.includes('Potash')) query = "Muriate of Potash fertilizer bag";
    ...
    else if (query.includes('Zinc') || query.includes('Ammonium') || query.includes('Calcium') || query.includes('Boron') || query.includes('Ferrous') || query.includes('Sulphur') || query.includes('Gypsum')) query = `${p.name.split('(')[0]} agricultural fertilizer bag`;
    ...
    else if (query.includes('Biofertiliser') || query.includes('Bacteria') || query.includes('Viride') || query.includes('Fluorescens')) query = `${p.name.split('(')[0]} biofertilizer bottle packet agriculture`;
    else if (query.includes('Tonic') || query.includes('Jeevamrut')) query = `${p.name.split('(')[0]} organic liquid fertilizer bottle`;
    ...
    else if (query.includes('Irrigation') || query.includes('Sprinkler')) query = `${p.name.split('1 Acre')[0]} agriculture field`;
    else if (query.includes('Film') || query.includes('Net')) query = `${p.name.split('(')[0]} agriculture field`;
    else if (query.includes('Kit') || query.includes('Meter')) query = `${p.name} agriculture instrument`;
    else if (query.includes('Sickle') || query.includes('Hoe') || query.includes('Spade') || query.includes('Secateur')) query = `${p.name} agriculture hand tool`;
    else if (query.includes('Chaff Cutter')) query = "electric chaff cutter machine agriculture";
    else if (query.includes('Pump')) query = `${p.name.split('5 HP')[0].split('3 HP')[0]} agriculture water pump`;
    else if (query.includes('Tarpaulin') || query.includes('Gunny') || query.includes('Silo') || query.includes('Scale') || query.includes('Trough') || query.includes('Drinker') || query.includes('Wheelbarrow') || query.includes('Fogger') || query.includes('Trap')) query = `${p.name.split('(')[0]} agriculture equipment`;
