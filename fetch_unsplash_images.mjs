  ...
  for (const row of rowsRaw) {
    ...
    if (titleMatch) {
      ...
      let query = title
        .replace(/\(.*?\)/g, "")
        .trim()
        .toLowerCase(); // e.g. "paddy seed pr-126"
      ...
      if (query.includes("seed")) {
        ...
        query.includes("dap") ||
        query.includes("npk") ||
        query.includes("potash") ||
        query.includes("phosphate") ||
        query.includes("sulphate") ||
        query.includes("nitrate")
      ) {
        query = "fertilizer agriculture bag";
      } else if (
        query.includes("vermicompost") ||
        query.includes("manure") ||
        query.includes("bone meal") ||
        query.includes("neem cake")
      ) {
        query = "organic compost agriculture";
      } else if (
        query.includes("biofertiliser") ||
        query.includes("bacteria") ||
        query.includes("mycorrhiza") ||
        query.includes("pseudomonas") ||
        query.includes("trichoderma")
      ) {
        query = "soil agriculture farm";
      } else if (
        query.includes("acid") ||
        query.includes("extract") ||
        query.includes("micronutrient") ||
        query.includes("boron") ||
        query.includes("tonic") ||
        query.includes("concentrate") ||
        ...
        query.includes("pump") ||
        query.includes("controller")
      ) {
        query = "agriculture water pump";
      } else if (
        query.includes("weeder") ||
        query.includes("cutter") ||
        query.includes("rotavator") ||
        query.includes("drill") ||
        query.includes("trolley")
      ) {
        query = "tractor agriculture equipment";
      } else if (
        query.includes("irrigation") ||
        query.includes("sprinkler") ||
        query.includes("pipe") ||
        query.includes("mulching") ||
        query.includes("net")
      ) {
      ...
      try {
        ...
        if (url) {
          ...
          mappings[title] =
            "https://images.unsplash.com/photo-1595841696650-6a75f8f53702?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080";
        }
      } catch (err) {
        mappings[title] =
          "https://images.unsplash.com/photo-1595841696650-6a75f8f53702?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080";
